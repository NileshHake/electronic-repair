...
// Optional: helper to get feature data outside component
export const getFeatureData = () => {
  const { data } = useGetStoreFeatureListQuery();
  return data?.data || [];
};
    ...
    content = (
      <div className="row align-items-center">
        {data.data.map((item, i) => {
          const IconComponent = ICONS[item.icon]; // dynamic icon from ICONS
  ...
  return (
    <section className="tp-feature-area tp-feature-border-2 pb-80">
      <div className="container">
        <div className="tp-feature-inner-2">{content}</div>
      </div>
    </section>
  );
};
