...
const OrderViewModal = ({ isOpen, toggle, order }) => {
  // ✅ derive safely even if order is null (NO early return before hooks)
  const orderId = order?.order_master_id;

  // ✅ QUERY proper usage (auto fetch + skip support)
  const { data, isLoading, isError } = useGetOrderChildListQuery(
    { order_id: orderId },
    { skip: !isOpen || !orderId }
  );
  ...
  // ✅ conditional render AFTER hooks (fixes Next build ESLint error)
