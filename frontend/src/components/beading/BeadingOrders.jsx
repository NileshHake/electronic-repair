...
const BeadingOrders = () => {
  ...
  // ✅ which request's offers to load
  const [offersReqId, setOffersReqId] = useState(null);
  ...
  // ✅ vendor offers query (runs when offersReqId is set)
  const {
    data: vendorOffersRes,
    isFetching: offersLoading,
    isError: offersError,
  } = useGetVendorOffersByRequestQuery(offersReqId, {
    skip: !offersReqId,
  });
  ...
  const openModal = (row) => {
    ...
    setActive({
      ...row,
      vendorOffers: [],
      vendorOffersLoading: true, // optional flag for modal
    });

    // ✅ trigger API
    setOffersReqId(row?.beading_request_id);
  };

  // ✅ when vendor offers fetched, store into active.vendorOffers
  useEffect(() => {
    if (!open) return;
    if (!offersReqId) return;

    // if API returns direct array
    const offersArray = Array.isArray(vendorOffersRes)
      ? vendorOffersRes
      : Array.isArray(vendorOffersRes?.data) // if API returns {data:[]}
      ? vendorOffersRes.data
      : [];

    setActive((prev) => {
      if (!prev) return prev;
      // only update if same active request
      if (String(prev?.beading_request_id) !== String(offersReqId)) return prev;

      return {
        ...prev,
        vendorOffers: offersArray,
        vendorOffersLoading: false,
        vendorOffersError: !!offersError,
      };
    });
  }, [vendorOffersRes, offersError, open, offersReqId]);

  // ✅ also reflect loading state to modal
  useEffect(() => {
    if (!open) return;
    setActive((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
        vendorOffersLoading: offersLoading,
      };
    });
  }, [offersLoading, open]);
  ...
  return (
    <>
                    ...
                    return (
                      <tr key={o?.beading_request_id || idx}>
                        ...
                        <td style={{ minWidth: 180 }}>
                          ...
                          <div className="text-muted small" title={o?.beading_request_description || ""}>
      ...
      <BeadingDetailsModal open={open} onClose={closeModal} data={active} imgBaseUrl={baseURL} />
    </>
  );
};
