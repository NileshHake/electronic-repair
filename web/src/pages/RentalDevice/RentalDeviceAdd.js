...
const RentalDeviceAdd = ({ isOpen, toggle }) => {
    ...
    const { addRentalDeviceResponse } = useSelector(
    ...
    const [galleryFiles, setGalleryFiles] = useState([]); // [{file, preview, name, formattedSize}]
    ...
    const subCategoryOptions = useMemo(
        () =>
            (subCategories || []).map((s) => ({
                value: s.category_id, // change if your key different
                label: s.category_name, // change if your key different
            })),
        [subCategories]
    );
    ...
    const selectedCategory = categoryOptions.find(
        (o) => String(o.value) === String(form.device_category)
    ) || null;
    ...
    const selectedBrand = brandOptions.find(
        (o) => String(o.value) === String(form.device_brand )
    ) || null;
    ...
    const validate = () => {
        ...
        if (day <= 0 && week <= 0 && month <= 0) {
            e.base_rent_per_day = "Add rent (day/week/month)";
        }

        if (Number(form.min_rental_days || 0) <= 0) e.min_rental_days = "Min rental days must be 1+";

        // images optional — if you want required then uncomment:
        // if (galleryFiles.length === 0) e.images = "At least 1 image is required";
    ...
    const buildFormData = () => {
        ...
        // ✅ normal fields
        fd.append("device_category", form.device_category);
        fd.append("device_sub_category_id", form.device_sub_category_id);
        fd.append("device_brand ", form.device_brand );
        ...
        if (form.base_rent_per_day !== "") fd.append("base_rent_per_day", form.base_rent_per_day);
        if (form.base_rent_per_week !== "") fd.append("base_rent_per_week", form.base_rent_per_week);
        if (form.base_rent_per_month !== "") fd.append("base_rent_per_month", form.base_rent_per_month);

        if (form.security_deposit !== "") fd.append("security_deposit", form.security_deposit);
        ...
        galleryFiles.forEach((x) => {
            fd.append("images", x.file); // backend should read req.files.images (array)
        });
    ...
    const handleSubmit = (e) => {
        ...
        dispatch(addRentalDevice(fd)); // ✅ send FormData
    };
                        ...
                        <Card className="border card-border-success p-3 shadow-lg">
                            <Row className="g-3">
                                ...
                                <Col md="4">
                                    ...
                                    <Select
                                        ...
                                        onChange={(opt) => {
                                            setField("device_category", opt ? opt.value : "");
                                ...
                                <Col md="4">
                                    <Label className="d-flex justify-content-between">
                                        ...
                                        {error.device_sub_category_id && <div className="text-danger text-sm">{error.device_sub_category_id}</div>}
                                    </Label>
                                    <Select
                                ...
                                <Col md="3">
                                    <Label className="d-flex justify-content-between">
                                        <div>Rent / Day</div>
                                        {error.base_rent_per_day && (
                                            <div className="text-danger text-sm">{error.base_rent_per_day}</div>
                                        )}
                                    </Label>
                                    <Input
                                        type="number"
                                ...
                                <Col md="3">
                                    <Label className="d-flex justify-content-between">
                                        ...
                                        {error.min_rental_days && (
                                            <div className="text-danger text-sm">{error.min_rental_days}</div>
                                        )}
                                    </Label>
                                    <Input
                                        type="number"
                                ...
                                <Col md="3">
                                    ...
                                    <Select
                                        ...
                                        onChange={(option) =>
                                            setField("delivery_available", option ? option.value : 0)
                                        }
                                ...
                                <Col md="3">
                                    ...
                                    <Select
                                        ...
                                        onChange={(option) =>
                                            setField("status", option ? option.value : "")
                                        }
                            ...
                            <Card className="mt-3 border-0 shadow-sm">
                                <CardBody>
                                    ...
                                    {error.images && <div className="text-danger mb-2">{error.images}</div>}
                                    ...
                                    >
                                        {({ getRootProps, getInputProps }) => (
                                            <div className="dropzone dz-clickable" {...getRootProps()}>
                                                ...
                                                {/* ✅ Only opens file manager for images */}
                                                <input {...getInputProps()} />
                                            </div>
                                        )}
                                    </Dropzone>
