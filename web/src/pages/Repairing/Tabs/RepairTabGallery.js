...
const RepairTabGallery = ({ form }) => {
  ...
  // ✅ cleanup blob previews (memory leak fix)
  useEffect(() => {
    return () => {
      (galleryFiles || []).forEach((f) => {
        if (f?.isExisting) return; // existing image is URL from server
      ...
      <Col lg={12}>
        <Card className="mb-4">
          <CardBody>
            ...
            <p className="text-muted">Add Repair Images.</p>
