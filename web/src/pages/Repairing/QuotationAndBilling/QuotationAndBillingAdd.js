...
import { addQuotationBilling, getSingleQuotationBilling } from "../../../store/QuotationAndBilling";

const QuotationAndBillingAdd = ({ isOpen, toggle, RepairData, type }) => {
    ...
    useEffect(() => {
        ...
        // ================================
        if (RepairData && type === "Billing") {
            dispatch(getSingleQuotationBilling(RepairData.repair_quotation_id));
        }
    }, [dispatch, RepairData]);

    // When singleQuotationBilling updates → Load items
  useEffect(() => {
    if (type === "Billing" && singleQuotationBilling?.items) {
...
const handleProductAdd = (product) => {
    setInvoiceItems((prev) => {
        const updated = [...prev];
        const index = updated.findIndex((item) => item.product_id === product.product_id);
        ...
        if (index !== -1) {
            ...
            updated[index] = {
                ...
                quotation_and_billing_child_total: productTotal + taxValue,
                updated_at: now,
            };
        ...
        const newItem = {
            product_id: product.product_id, // Original product ID
            quotation_and_billing_child_product_id: product.product_id, // Unique invoice row ID
    ...
    const updateRow = (index, field, value) => {
        ...
        updated[index].quotation_and_billing_child_total = base + gstValue;
                                        ...
                                        invoiceItems.map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                {item.quotation_and_billing_service_or_product === "Service" ? (
                                                    <>
                                                        <td>
                                                            <Input value={item.quotation_and_billing_product_name}
                                                        ...
                                                        <td>0</td>
                                                        <td>
                                                            <Input type="number" value={item.quotation_and_billing_product_sale_price}
                                                    ...
                                                    <>
                                                        ...
                                                        <td>
                                                            <Input type="number" value={item.quotation_and_billing_tax_percentage}
