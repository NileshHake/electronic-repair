...
export const getRentalRequestList = () => ({
...
export const RentalRequestReducer = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case API_RESPONSE_SUCCESS:
      switch (action.payload.actionType) {
        case GET_RENTAL_REQUEST_LIST:
          return {
            ...state,
            rentalRequests: action.payload.data,
            loading: false,
            error: false,
          };

        case ADD_RENTAL_REQUEST:
          return { ...state, addRentalRequestResponse: true, loading: false, error: false };

        case UPDATE_RENTAL_REQUEST:
          return { ...state, updateRentalRequestResponse: true, loading: false, error: false };
    ...
    case API_RESPONSE_ERROR:
      return {
        ...state,
        addRentalRequestResponse: null,
        error: true,
        loading: false,
      };

    case RESET_ADD_RENTAL_REQUEST_RESPONSE:
      return { ...state, addRentalRequestResponse: false };

    case RESET_UPDATE_RENTAL_REQUEST_RESPONSE:
      return { ...state, updateRentalRequestResponse: false };
...
const getRentalRequestApi = () => api.get("/rental-request/list");
const addRentalRequestApi = (data) => api.create("/rental-request/store", data);
const updateRentalRequestApi = (data) => api.put("/rental-request/update", data);
const deleteRentalRequestApi = (id) => api.delete(`/rental-request/delete/${id}`);

/* ================== SAGAS ================== */
function* getRentalRequestListSaga() {
  try {
    const response = yield call(getRentalRequestApi);
    yield put(rentalRequestApiResponseSuccess(GET_RENTAL_REQUEST_LIST, response));
  } catch (error) {
    yield put(rentalRequestApiResponseError(GET_RENTAL_REQUEST_LIST, error));
...
function* addRentalRequestSaga({ payload }) {
  try {
    ...
    yield put(rentalRequestApiResponseSuccess(ADD_RENTAL_REQUEST, response));
    yield call(getRentalRequestListSaga);
    toast.success("Rental request created successfully!");
  } catch (error) {
    yield put(rentalRequestApiResponseError(ADD_RENTAL_REQUEST, error));
...
function* updateRentalRequestSaga({ payload }) {
  try {
    ...
    yield put(rentalRequestApiResponseSuccess(UPDATE_RENTAL_REQUEST, response));
    yield call(getRentalRequestListSaga);
    toast.success("Rental request updated successfully!");
  } catch (error) {
    yield put(rentalRequestApiResponseError(UPDATE_RENTAL_REQUEST, error));
...
function* deleteRentalRequestSaga({ payload }) {
  try {
    ...
    yield put(rentalRequestApiResponseSuccess(DELETE_RENTAL_REQUEST, { data: payload }));
    yield call(getRentalRequestListSaga);
    toast.success("Rental request deleted successfully!");
  } catch (error) {
    yield put(rentalRequestApiResponseError(DELETE_RENTAL_REQUEST, error));
